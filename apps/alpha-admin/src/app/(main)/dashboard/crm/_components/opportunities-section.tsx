...
export function OpportunitiesSection() {
  ...
  const stageFilter =
    (table.getColumn('stage')?.getFilterValue() as string | undefined) ?? 'all'
  const healthFilter =
    (table.getColumn('health')?.getFilterValue() as string | undefined) ?? 'all'
                ...
                {table.getRowModel().rows.length ? (
                  table.getRowModel().rows.map((row) => (
                    <TableRow
                      key={row.id}
                      data-state={
                        table.state.rowSelection[row.id] && 'selected'
                      }
                    >
                ...
                {pageNumbers.map((pageNumber) => (
                  <PaginationItem key={`page-${pageNumber}`}>
                    <PaginationLink
                      href="#"
                      isActive={
                        table.state.pagination.pageIndex === pageNumber - 1
                      }
                      onClick={(event) => {
                        preventPaginationNavigation(event)
