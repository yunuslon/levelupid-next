...
import { MONITORING_SPEED, createRealtimeTicker } from './realtime-utils'
...
const compactWaveformTicker = createRealtimeTicker(
  COMPACT_WAVEFORM_TICK_INTERVAL_MS,
)
const detailWaveformTicker = createRealtimeTicker(
...
export function useWaveformTick(compact: boolean) {
  ...
  return (
    useSyncExternalStore(ticker.subscribe, ticker.getSnapshot, getInitialTick) *
    monitoringSpeed
  )
}
...
export function useTrendTick() {
  return Math.floor(
    useSyncExternalStore(
      trendTicker.subscribe,
      trendTicker.getSnapshot,
      getInitialTick,
    ) * monitoringSpeed,
  )
}
