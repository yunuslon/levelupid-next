...
import {
  type TrendKind,
  type TrendPoint,
  usePatientTrendSeries,
} from './use-patient-vital-series'
...
function VitalTrendChart({
  ...
  return (
    <ChartContainer
      ...
      className={cn(
        'aspect-auto h-full min-h-12 w-full border-current border-r',
        trendClasses[kind],
      )}
...
function TrendStrip({
  ...
  return (
    <div className="grid min-h-12 grid-cols-[4rem_2rem_minmax(0,1fr)_2.5rem] items-stretch gap-1 border-border border-b">
      ...
      <div
        className={cn(
          'self-end pr-1 text-right font-medium text-sm tabular-nums',
          color,
        )}
      >
        {value}
      ...
      <div className="px-3 py-2">
        ...
        <div className="flex flex-col gap-2">
          {patient.recentEvents.map((event) => (
            <div
              className="grid grid-cols-[3rem_1fr] text-[11px]"
