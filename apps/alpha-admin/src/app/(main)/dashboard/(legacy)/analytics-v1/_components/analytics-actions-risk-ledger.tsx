...
const ledgerColumns: ColumnDef<DataTableFeatures, LedgerRow>[] = [
  ...
  },
  {
    ...
    header: 'Idle (days)',
    cell: ({ row }) => (
      <span className="text-xs tabular-nums">{row.original.idleDays}d</span>
    ),
  },
  {
    ...
    cell: ({ row }) => (
      <span className="text-xs tabular-nums">{row.original.closeVariance}</span>
    ),
  },
  {
      ...
      <div className="flex max-w-64 flex-col gap-1 whitespace-normal">
        {row.original.priority ? (
          <Badge
            ...
            className={cn(
              'text-[10px] uppercase',
              priorityTone[row.original.priority],
            )}
          >
            {row.original.priority}
          </Badge>
  ...
  return (
    <Card className="min-w-0 shadow-xs">
      <CardHeader>
        ...
        <CardDescription>
          Accounts under pressure with blocker, next action, and owner
          responsibility.
        </CardDescription>
        <CardAction>
      ...
      <CardContent className="space-y-4">
        <div className="grid gap-3 rounded-lg border bg-muted/20 p-3 text-sm sm:grid-cols-4 sm:divide-x sm:divide-border/60">
          <LedgerStat
            ...
            value="1"
            detail="Risk Ladder >= 80 (current window)"
          />
