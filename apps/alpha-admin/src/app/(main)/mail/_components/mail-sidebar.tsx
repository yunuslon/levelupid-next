...
import { type MailNavItem, accounts, mailNavigation } from './data'
    ...
    >
      <SidebarHeader className="gap-3 py-3 pb-1">
        ...
        <div className="flex flex-col gap-1.5 group-data-[state=collapsed]:hidden">
          ...
          <div className="truncate text-muted-foreground text-sm leading-none">
      ...
      <SidebarContent>
        <SidebarGroup>
          <SidebarMenu className="gap-1">
            {mailNavigation.navMain.map(renderNavItem)}
          </SidebarMenu>
        </SidebarGroup>
        ...
        <SidebarGroup>
          ...
          <SidebarMenu className="gap-1">
            {mailNavigation.folders.map(renderNavItem)}
          </SidebarMenu>
        </SidebarGroup>
      </SidebarContent>
      <SidebarFooter>
        <SidebarMenu className="gap-1">
          {mailNavigation.navFooter.map(renderNavItem)}
        </SidebarMenu>
      </SidebarFooter>
    </Sidebar>
  )
...
function renderNavItem(nav: MailNavItem) {
  return (
    <SidebarMenuItem key={nav.id}>
      ...
      {nav.label && (
        <SidebarMenuBadge className="font-medium">{nav.label}</SidebarMenuBadge>
      )}
    </SidebarMenuItem>
  )
}
