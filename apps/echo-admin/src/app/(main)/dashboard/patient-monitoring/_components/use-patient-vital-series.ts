...
const trendDefinitions = {
  'heart-rate': {
    baseline: 'heartRate',
    domain: [50, 150],
    getVariation: (patient) =>
      heartRateVariations[patient.signalProfile.rhythm],
    phaseOffset: 0,
    ticks: [50, 100, 150],
  },
  map: {
...
function repeatTemplate(
  ...
  const positionInTemplate =
    ((cyclePosition % cyclesPerTemplate) + cyclesPerTemplate) %
    cyclesPerTemplate
...
function getSignalValue(
  ...
  if (kind === 'respiration') {
    return repeatTemplate(
      respirationTemplate,
      time * (patient.respirationRate / 60) + phase,
    )
  }
  ...
  if (kind === 'arterial') {
    const [systolic = 120, diastolic = 70] = patient.arterialPressure
      .split('/')
      .map(Number)
    return (
      diastolic +
      repeatTemplate(arterialTemplate, beatPosition) * (systolic - diastolic)
    )
  }
  ...
  return (
    repeatTemplate(template.samples, beatPosition, template.beats) *
    amplitude *
    leadScale
  )
}
...
function getTrendValue(
  ...
  const patientOffset = Math.round(
    patient.signalProfile.phase * normalizedTrendTemplate.length,
  )
  const position = sample + patientOffset + definition.phaseOffset
  const index =
    ((position % normalizedTrendTemplate.length) +
      normalizedTrendTemplate.length) %
    normalizedTrendTemplate.length
  const templateValue = normalizedTrendTemplate[index] ?? 0
  const value =
    patient[definition.baseline] +
    templateValue * definition.getVariation(patient)
...
export function usePatientTrendSeries({
  kind,
  patient,
}: PatientTrendSeriesOptions) {
  const tick = useTrendTick()
  const definition = trendDefinitions[kind]
  const data = useMemo(
    () => createTrendWindow(patient, definition, tick),
    [definition, patient, tick],
  )
