...
const FILTER_OPTIONS: Array<{
...
}> = [
  {
    ...
    summaryLabel: 'Enterprise',
  },
  {
    key: 'stalledOnly',
    label: 'Stalled deals (>14 days)',
    summaryLabel: 'Stalled',
  },
  {
    key: 'overdueOnly',
    label: 'Closing date exceeded',
    summaryLabel: 'Overdue',
  },
  {
    key: 'includeRenewals',
    label: 'Include renewals',
    summaryLabel: 'Renewals',
  },
]
...
export function AnalyticsOverview() {
  ...
  const handleDateRangeChange = (value: DateRange | undefined) => {
    ...
    setRevenueSeries(
      buildRevenueChartData(nextDateRange.from, nextDateRange.to),
    )
  }
  return (
...
function buildRevenueChartData(from: Date, to: Date) {
  ...
  return days.map((day) => {
    const nextRevenue =
      currentRevenue + Math.round((Math.random() - 0.45) * 4_000)
...
function FiltersPopover({
  ...
  return (
    <div className="flex items-center gap-2">
      <Popover open={open} onOpenChange={setOpen}>
        <PopoverTrigger
          render={<Button variant="outline" aria-expanded={open} />}
        >
          Filters
      ...
      <span className="text-muted-foreground text-sm">
        ...
        <span className="font-medium">
          {summarizeFilterState(selectedFilters)}
        </span>
      </span>
    </div>
  )
